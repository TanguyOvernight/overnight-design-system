...
Le cœur (Heuer, *Psychology of Intelligence Analysis*, CIA 1999, ch. 8) : on
ne prouve pas une thèse en empilant des preuves POUR — presque toute preuve
est compatible avec plusieurs récits. On retient l'hypothèse qui a le MOINS
de preuves CONTRE, après avoir cherché à infirmer CHACUNE. [ROBUSTE comme
logique (inférence éliminatoire) ; DÉBATTU comme outil : Dhami et al. 2019
et une étude 2024 (N=161) montrent que la matrice seule ne réduit pas
fiablement le biais de confirmation et reste sensible au choix des preuves ;
la RAND (RR-1408) note le déficit d'évaluation empirique des SAT. → Garder
les 3 gestes robustes : expliciter les rivales, chercher l'infirmant, jeter
le non-diagnostique.]
...
1. **Diagnosticité d'abord** : un fait C avec les 4 (« le marché croît »,
   « le management est confiant », « le produit est bon ») pèse ZÉRO —
   pseudo-diagnostique, interdit à l'appui du verdict. Réflexe : demander
   P(fait | thèse FAUSSE) — la question que le cerveau saute spontanément
   (pseudodiagnosticité, Doherty et al. 1979) [ROBUSTE].
2. **Verdict = l'hypothèse la moins infirmée**, jamais la plus « soutenue ».
3. **Sensibilité** (étapes 6 et 8 de l'ACH original) : nommer les 2-3 faits
   qui portent SEULS la discrimination → re-vérification prioritaire ; s'ils
   sont ⛔ le verdict plafonne (règle existante) ; ce sont eux qui donnent
   les vérifications DATÉES d'un AFFAIBLI (« la publication du JJ/MM
   tranche H1 vs H2 ») et le kill criterion d'un CONFIRMÉ.

### Key Assumptions Check + Quality of Information Check (Tradecraft Primer, CIA 2009)
- **KAC** : écrire 3-5 prémisses implicites du signal (« le management dit
  vrai », « la demande continue », « le multiple historique est la bonne
  référence », « le précédent Y est comparable ») et classer : soutenue /
  correcte avec réserves / non soutenue. ≥ 2 prémisses non soutenues sous
  un fait central = château de cartes, quel que soit le nombre de C.
- **QIC** : pour chaque source-clé — QUI le dit ? COMMENT le sait-il
  (témoin direct, calcul, ouï-dire) ? Quel TRACK RECORD ? Séparer fait
  rapporté / interprétation / rumeur reprise. L'étape 1 vérifie le fait ;
  le QIC vérifie la CHAÎNE qui l'a transporté.

### Détection de déception — MOM / POP / MOSES / EVE (Pherson & Heuer)
À dérouler dès que H3 est plausible :
- **MOM** — Motif (levée/refi à venir, vesting, short à faire couvrir,
  gérant qui parle de son livre) · Opportunité (contrôle du canal : IR,
  chiffres non audités, KPI maison invérifiables) · Moyens (historique de
  communication agressive).
- **POP** — précédents du même émetteur : restatements, sanctions,
  promesses ratées (meilleur prédicteur d'une déception future).
- **MOSES** — ma source est-elle manipulable ? (embargo offert, accès
  privilégié, « exclusivité » = le journaliste a été CHOISI).
- **EVE** — la preuve est-elle vérifiable indépendamment du déclarant ?
Règle : un MOM élevé ne rend pas le fait FAUX — il DÉGRADE son poids
(LR → 1) et hausse l'exigence de preuve (étape 4). Jamais de RÉFUTÉ sur la
seule incitation.
...
5-10 pts (science-prevision, règle 6).

Quatre règles d'emploi :
1. **Anti-double-comptage** [ROBUSTE — pathologie documentée en
   méta-analyse, Senn 2009] : la question n'est pas « combien de sources ? »
   mais « combien d'ORIGINES ? ». 3 articles + 1 tweet citant le même
   communiqué = UNE preuve. Avant tout point d'ajustement : tracer la
   chaîne jusqu'à l'origine ; même origine = déjà compté.
...
4. **Budget** : le cumul reste dans le ±15 pts de la charte ; la table
   RÉPARTIT ce budget. 12 preuves faibles corrélées < 2 preuves
   indépendantes coûteuses.
...
- **NOA bloat** (Hirshleifer-Hou-Teoh-Zhang, JAE 2004) [ROBUSTE en
  ...
  NOA = (actifs − cash/placements) − (passifs − dette), rapporté aux actifs
  N-1. NOA élevés = des ANNÉES d'accruals cumulés jamais validés par le
  cash — prédicteur négatif des rendements sur ≥ 3 ans (« sustainability
  effect »). Sloan (en base) voit UNE année ; les NOA voient l'historique.
  Vérif : NOA/actifs vs pairs + trajectoire 3 ans.
- **Décomposer les accruals avant d'accuser** : part « normale » (suit la
  ...
  qualité d'un accrual = finit-il en cash ? Vérif praticable : les
  créances/stocks d'il y a 2 ans sont-ils devenus du CFO — ou des
  write-offs « exceptionnels » (aveu rétroactif de résultats gonflés) ?
...
- **SaaS / abonnement** [praticien, Maxio 2026] : (1) billings implicites
  ...
  croissance affichée ; (4) marge brute > 10 pts au-dessus des pairs ;
  (5) développement capitalisé > ~40 % de la R&D totale — (4)+(5) = deux
  drapeaux de maquillage.
- **Industrie / contrats longs (percentage-of-completion)** : (1) marges
  de contrat étrangement CONSTANTES en période de coûts volatils = les
  estimations de coûts totaux sont pilotées [ForensicRisk] ; (2) unbilled
  receivables / contract assets qui gonflent plus vite que le CA = revenu
  reconnu mais pas facturable (earnings management documenté) ; (3)
  révisions de coûts toujours tardives et groupées = pertes retenues.
- **Retail / stocks** [Schilit, *Financial Shenanigans*] : (1) décomposer
  l'inventaire (note annexe) — produits FINIS qui gonflent plus vite que
  ...
  démarques/dépréciations pas passées ; (3) rotation des stocks vs pairs.
...
- **Loughran-McDonald (JF 2011)** [ROBUSTE] : dictionnaires FINANCIERS
  ...
  du dictionnaire Harvard ne le sont pas en finance. Hausse des mots
  d'incertitude/litigieux ↔ fraudes, faiblesses de contrôle, surprises.
  Usage sans NLP : comparer le MÊME document N vs N-1 — nouveaux hedges
  (« substantial doubt », « covenant »), risk factors qui gonflent, et la
  métrique-star (ARR, NRR, cohortes) qui DISPARAÎT = elle s'est retournée.
- **Li (JAE 2008)** [ROBUSTE sur la direction] : rapports plus longs et
  moins lisibles → résultats MOINS persistants ; l'obfuscation est un
  choix qui cache la non-durabilité. (La métrique Fog est critiquée en
  finance — Loughran-McDonald JF 2014 préfèrent des proxys simples type
  longueur du document [DÉBATTU sur la métrique, pas sur l'effet].)
- **Larcker-Zakolyukina (JAR 2012)** [ROBUSTE en drapeau, DÉBATTU en
  exploitation directe] : sur les calls suivis de restatements, les
  dirigeants « déceptifs » font PLUS de références au savoir général
  (« comme chacun sait ») et d'émotions positives EXTRÊMES (« fantastic »),
  MOINS de mots d'anxiété et de références à la valeur actionnariale.
  Modèle > hasard de 6-16 % ; portefeuille high-deception : alpha −4 à
  −11 %/an. Usage : Q&A esquivé + superlatifs + « nous » impersonnel =
  drapeau qui déclenche le forensic chiffré ci-dessus.

## Intégration à la charte (renvois)
- La matrice H1-H4 s'instruit à l'étape 3 (thèse inverse) : H2 est déjà
  l'étape 2, H3 l'étape 4, H4 l'étape 1 — l'ACH les fait converger vers UN
  verdict au lieu de 6 attaques parallèles.
- La table LR discipline l'ajustement ±15 pts de la bibliothèque de base
  rates ; l'anti-double-comptage protège aussi la règle 1 de la charte
  (l'asymétrie déjà encodée ne doit pas être recomptée).
- Le crible causal s'applique au steelman (étape 0) et à la thèse inverse.
- Le forensic sectoriel étend l'étape 4 ; les vérifications datées d'un
  AFFAIBLI sortent de l'analyse de sensibilité ACH (les 2-3 faits porteurs).

Sources principales : CIA Tradecraft Primer 2009 (cia.gov) · Heuer 1999
ch. 8 · Dhami-Belton-Mandel, Applied Cognitive Psychology 2019 · Good,
« Weight of Evidence » 1985 · Senn, BMC 2009 (double comptage) · Rosenzweig,
*The Halo Effect* 2007 · Kahneman-Lovallo 1993 · Hirshleifer et al., JAE
2004 · Dechow-Dichev, TAR 2002 · Loughran-McDonald, JF 2011 & 2014 · Li,
JAE 2008 · Larcker-Zakolyukina, JAR 2012 · Schilit, *Financial Shenanigans*.
URLs complètes : rapport de recherche du 27/07/2026 (journal).
